        ...
        if (axis === "x") {
            ...
            function calcY() {
                ...
                if (step === 0) {
                    ...
                    yVal = sizes.verticalStep * 3 + 48;
                } else {
                    yVal = sizes.verticalStep * step + 48;
                }
            }
                ...
                if (sizes.usedSpots[0]) {
                    for (let spot of sizes.usedSpots) {
                        ...
                        if (obj.x === spotXVal && yVal === spotYVal) {
                            ...
                            setY();
                            break;
                        } //prevent two gems/other future objects from landing on the same spot else
                    }
                }
            }
            setY();
            sizes.usedSpots.push({
                "x": obj.x,
                "y": yVal
            });
            return yVal;
...
let designRelated = {
    counterX: {
        jewelCount: function() {
            if (designRelated.counterWidth.jewelCount) {
                return (80 - 0.25 * designRelated.counterWidth.jewelCount)
            } else {
                return 75
            }
        },
        ...
        livesCount: function() {
            if (designRelated.counterWidth.jewelCount) {
                return (405 - 0.52 * designRelated.counterX.jewelCount())
            } else {
                return 380
            }
        }
...
Player.prototype.update = function(dt) {
    ...
    if (Rock.prototype.collision(this.x)) {
        this.x = this.oldX;
        this.y = this.oldY;
...
Player.prototype.render = function(dt) {
    ...
    if (key && key.collision(this.x)) {
        ...
        if (lockIndex1 !== -1) {
            stillObjects.splice(lockIndex1, 1)
        };
        let lockIndex2 = allRocks.indexOf(lock);
        if (lockIndex2 !== -1) {
            allRocks.splice(lockIndex2, 1)
        };
        orderAllObjects();
    }
    ...
    switch (keyCode) {
        ...
        case 'x':
            if (Player.prototype.changePlayerSprite) {
                ...
                Player.prototype.changePlayerSprite = setTimeout(function() {
                    Player.prototype.changePlayerSprite = false
                }, 400);
            } else {
                Player.prototype.changePlayerSprite = setTimeout(function() {
...
document.addEventListener('keyup', function(e) {
    ...
    if (obtained.gameOver) {
        player.handleInput(allowedKeys[e.keyCode]);
    } else {
        if (allowedKeys[e.keyCode] === 'z') {} else {
            player.handleInput(allowedKeys[e.keyCode]);
        }
    }
